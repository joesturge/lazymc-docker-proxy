...
impl Config {
    ...
    pub fn from_env<T: Adapter>() -> Self {
        ...
        if let Ok(value) = var("LAZYMC_GROUP") {
            ...
            T::stop(&value);
            labels.insert(String::from("lazymc.group"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_METHODS") {
            labels.insert(String::from("lazymc.join.methods"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_KICK_STARTING") {
            labels.insert(String::from("lazymc.join.kick.starting"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_KICK_STOPPING") {
            labels.insert(String::from("lazymc.join.kick.stopping"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_HOLD_TIMEOUT") {
            labels.insert(String::from("lazymc.join.hold.timeout"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_FORWARD_ADDRESS") {
            labels.insert(String::from("lazymc.join.forward.address"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_FORWARD_SEND_PROXY_V2") {
            labels.insert(String::from("lazymc.join.forward.send_proxy_v2"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_LOBBY_TIMEOUT") {
            labels.insert(String::from("lazymc.join.lobby.timeout"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_LOBBY_MESSAGE") {
            labels.insert(String::from("lazymc.join.lobby.message"), value);
        }
        if let Ok(value) = var("LAZYMC_JOIN_LOBBY_READY_SOUND") {
            labels.insert(String::from("lazymc.join.lobby.ready_sound"), value);
        }
        if let Ok(value) = var("LAZYMC_LOCKOUT_ENABLED") {
            labels.insert(String::from("lazymc.lockout.enabled"), value);
        }
        if let Ok(value) = var("LAZYMC_LOCKOUT_MESSAGE") {
            labels.insert(String::from("lazymc.lockout.message"), value);
        }
        if let Ok(value) = var("LAZYMC_PORT") {
            labels.insert(String::from("lazymc.port"), value);
        }
        if let Ok(value) = var("MOTD_SLEEPING") {
            labels.insert(String::from("lazymc.motd.sleeping"), value);
        }
        if let Ok(value) = var("MOTD_STARTING") {
            labels.insert(String::from("lazymc.motd.starting"), value);
        }
        if let Ok(value) = var("MOTD_STOPPING") {
            labels.insert(String::from("lazymc.motd.stopping"), value);
        }
        if let Ok(value) = var("MOTD_FROM_SERVER") {
            labels.insert(String::from("lazymc.motd.from_server"), value);
        }
        if let Ok(value) = var("PUBLIC_PROTOCOL") {
            labels.insert(String::from("lazymc.public.protocol"), value);
        }
        if let Ok(value) = var("PUBLIC_VERSION") {
            labels.insert(String::from("lazymc.public.version"), value);
        }
        if let Ok(value) = var("SERVER_ADDRESS") {
            labels.insert(String::from("lazymc.server.address"), value);
        }
        if let Ok(value) = var("SERVER_BLOCK_BANNED_IPS") {
            labels.insert(String::from("lazymc.server.block_banned_ips"), value);
        }
        if let Ok(value) = var("SERVER_DIRECTORY") {
            labels.insert(String::from("lazymc.server.directory"), value);
        }
        if let Ok(value) = var("SERVER_DROP_BANNED_IPS") {
            labels.insert(String::from("lazymc.server.drop_banned_ips"), value);
        }
        if let Ok(value) = var("SERVER_FORGE") {
            labels.insert(String::from("lazymc.server.forge"), value);
        }
        if let Ok(value) = var("SERVER_PROBE_ON_START") {
            labels.insert(String::from("lazymc.server.probe_on_start"), value);
        }
        if let Ok(value) = var("SERVER_SEND_PROXY_V2") {
            labels.insert(String::from("lazymc.server.send_proxy_v2"), value);
        }
        if let Ok(value) = var("SERVER_START_TIMEOUT") {
            labels.insert(String::from("lazymc.server.start_timeout"), value);
        }
        if let Ok(value) = var("SERVER_STOP_TIMEOUT") {
            labels.insert(String::from("lazymc.server.stop_timeout"), value);
        }
        if let Ok(value) = var("SERVER_WAKE_WHITELIST") {
            labels.insert(String::from("lazymc.server.wake_whitelist"), value);
        }
        if let Ok(value) = var("TIME_MINIMUM_ONLINE_TIME") {
            labels.insert(String::from("lazymc.time.minimum_online_time"), value);
        }
        if let Ok(value) = var("TIME_SLEEP_AFTER") {
            labels.insert(String::from("lazymc.time.sleep_after"), value);
        }

        Config::from_container_labels(&labels)
    }

    pub fn env_to_label(environment: HashMap<String, String>) -> HashMap<String, String> {
        environment
            .into_iter()
